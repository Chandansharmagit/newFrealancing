...
import { User, MapPin, Briefcase, Globe, Edit2, Save, Upload, LogOut } from "lucide-react";
...
const UserProfile = () => {
      ...
      try {
        ...
        if (data.isAuthenticated && data.user) {
          const userData = {
            ...
            bio: data.user.bio || "Passionate explorer of hidden gems and vibrant cultures.",
            location: data.user.location || "Unknown Location",
            trips: data.user.trips || 0,
            countries: data.user.countries || 0,
            profilePic: data.user.profilePic || "https://mir-s3-cdn-cf.behance.net/project_modules/max_1200/35af6a41332353.57a1ce913e889.jpg",
          };
          setUser(userData);
    ...
    if (isEditing) {
      try {
        ...
        if (document.getElementById("tw-profile-upload").files[0]) {
          formData.append("profilePic", document.getElementById("tw-profile-upload").files[0]);
        }

        // Send PUT request to update profile
        const response = await fetch("https://authenticationagency.onrender.com/api/update-profile", {
          method: "PUT",
          credentials: "include",
          body: formData,
        });
        ...
        const updatedResponse = await fetch("https://authenticationagency.onrender.com/api/check-auth", {
          method: "GET",
          credentials: "include",
        });
        ...
        if (updatedData.isAuthenticated && updatedData.user) {
          const updatedUser = {
            ...
            bio: updatedData.user.bio || "Passionate explorer of hidden gems and vibrant cultures.",
            location: updatedData.user.location || "Unknown Location",
            trips: updatedData.user.trips || 0,
            countries: updatedData.user.countries || 0,
            profilePic: updatedData.user.profilePic || "https://mir-s3-cdn-cf.behance.net/project_modules/max_1200/35af6a41332353.57a1ce913e889.jpg",
          };
          setUser(updatedUser);
  ...
  const handleLogout = async () => {
    try {
      const response = await fetch("https://authenticationagency.onrender.com/logout", {
        method: "GET",
        credentials: "include",
      });
    ...
    return (
      <div className="tw-profile-page tw-error">
        <div className="tw-error-message">
          ...
          <p>{error}</p>
          <button onClick={() => window.location.reload()} className="tw-retry-button">
            Try Again
          </button>
        </div>
      </div>
      ...
      <nav className="tw-profile-nav">
        <div className="tw-nav-container">
          ...
          <div className="tw-nav-actions">
            <button onClick={handleLogout} className="tw-logout-button" aria-label="Logout">
              <LogOut size={18} />
              <span>Logout</span>
            </button>
          </div>
