...
const DestinationsGrid = () => {
  ...
  const fetchDestinations = async (signal) => {
    try {
      ...
      if (data.success) {
        const formattedDestinations = data.destinations
          .map(dest => ({
            id: dest._id,
            name: dest.title,
  ...
  useEffect(() => {
    const metaConfig = {
      ...
      canonical: !loading && !error ? 'https://travelsansar.com/destinations' : undefined,
  ...
  // Memoize displayed destinations
  const displayedDestinations = useMemo(() => destinations.slice(0, 3), [destinations]);

  // Render loading skeleton
  if (loading) {
    return (
      <section className="dg-destinations-section" aria-busy="true">
      ...
      <section className="dg-destinations-section">
        <div className="dg-container">
          <div className="dg-error-container">
            ...
            <button className="dg-btn-retry" onClick={retryFetch}>
